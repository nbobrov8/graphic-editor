...
class Group(GrafObject):
    ...
    def coord_group(self):
        ...
        for object in self.gr_list:
            object.relative_x1 = (object.frame.x1 - self.frame.x2) / (
                self.frame.x2 - self.frame.x1
            )
            object.relative_x2 = (object.frame.x2 - self.frame.x2) / (
                self.frame.x2 - self.frame.x1
            )
            object.relative_y1 = (object.frame.y1 - self.frame.y2) / (
                self.frame.y2 - self.frame.y1
            )
            object.relative_y2 = (object.frame.y2 - self.frame.y2) / (
                self.frame.y2 - self.frame.y1
            )
    ...
    def resize_object(self):
        ...
        for object in self.gr_list:
            object.frame.x1 = (
                object.relative_x1 * (self.frame.x2 - self.frame.x1) + self.frame.x2
            )
            object.frame.x2 = (
                object.relative_x2 * (self.frame.x2 - self.frame.x1) + self.frame.x2
            )
            object.frame.y1 = (
                object.relative_y1 * (self.frame.y2 - self.frame.y1) + self.frame.y2
            )
            object.frame.y2 = (
                object.relative_y2 * (self.frame.y2 - self.frame.y1) + self.frame.y2
            )
            if object.is_group:
